...
export default class Home extends Component {
  ...
  async componentDidMount() {
    ...
    this.setState({ qr_code_url: window.location.href + "?restaurant_id="+this.state.user.uid })
    this.getData();
  }
  authListener() {
  ...
  render() {
    var { user, qr_code_url, data, registration_step, tabIndex, foodTypeOption, foodName, foodPrice, non_veg, pure_jain, selectedFood, qr_code_url } = this.state;
    return (
        ...
        <div className="row p-4 main-page">
          <div className="right-dashboard col-12">
            ...
            <div className="float-right">
              ...
              <button type="button" className="btn bg-dark text-white m-1 float-right" disabled> {user ? user.uid : null} </button>
            </div>
            <br /><br />
            <hr />
            <button type="button" className="btn btn-outline-primary m-1" onClick={() => this.changeTab(0)}> Food </button>
            <button type="button" className="btn btn-outline-success m-1" onClick={() => this.changeTab(2)}>Menu</button>
            <button type="button" className="btn btn-outline-warning m-1" onClick={() => this.changeTab(3)}> Code</button>
            <hr />
            <br /><br />
            {
              tabIndex === 0 && data ?
                ...
                : tabIndex === 2 && data ?
                  <div className="my-menu">
                    ...
                    <div className="row">
                      ...
                      {
                        selectedFood > -1 ? <div className="col-9 menu-items">
                          <h3>Food Items</h3>

                          <table className="table table-bordered">
                            {
                              data.menu[selectedFood].items.length > 0 ? <thead>
                                <tr>
...

                              {
                                data.menu[selectedFood].items.map((item, index) => {
                                  return <tr key={index}>
                                    ...
                                    <td className="text-center">{item.veg ? "Veg" : "---"}</td>
                                    <td className="text-center">{item.pure_jain ? "Jain" : "---"}</td>
                                    <td className="text-center">{item.non_veg ? "Non Veg" : "---"}</td>
                  ...
                  <div className="qr-code">
                    ...
                    />
                    <br/>
                    <button type="button" className="btn btn-outline-success my-3" onClick={() => this.downloadQR()}>Download QR</button>
